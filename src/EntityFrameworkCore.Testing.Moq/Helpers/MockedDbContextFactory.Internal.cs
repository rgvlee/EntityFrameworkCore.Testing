...
internal class MockedDbContextFactory<TDbContext> : BaseMockedDbContextFactory<TDbContext> where TDbContext : DbContext
{
    ...
    public override TDbContext Create()
    {
        ...
        var queryProvider = new Mock<IAsyncQueryProvider>();
        queryProvider.Setup(x => x.CreateQuery<It.IsAnyType>(It.IsAny<Expression>())).Throws(new Exception("asdf"));
        dependenciesMock.Setup(m => m.QueryProvider).Returns(() => queryProvider.Object);

        var typeMappingSource = new Mock<ITypeMappingSource>();
        typeMappingSource.Setup(x => x.FindMapping(It.IsAny<Type>())).Returns(() => new CustomCoreTypeMapping());
        dependenciesMock.Setup(x => x.TypeMappingSource).Returns(() => typeMappingSource.Object);
